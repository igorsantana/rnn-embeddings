    ...
    def __init__(self, setups, k):
        ...
        for _, params, _ in setups:
            self.metrics[params] = {    'm2vTN': AlgoMetrics('m2vTN', params), 'sm2vTN': AlgoMetrics('sm2vTN', params),
                                        ...
                                        'rnnTN': AlgoMetrics('rnnTN', params) }
        self.final_df = pd.DataFrame()
        
    def fold_results(self, params, m2vTN, sm2vTN, csm2vTN, csm2vUK, rnnTN, fold):
        ...
        self.metrics[params]['rnnTN'].add(rnnTN)
        metrics = np.vstack([m2vTN, sm2vTN, csm2vTN, csm2vUK, rnnTN])
        print()
        data = {    
            'params': [params] * 5,
            'algo': ['m2vTN','sm2vTN','csm2vTN','csm2vUK', 'rnnTN'],
            'folds':[fold] * 5,
            'prec': metrics[:,0],
            'rec': metrics[:,1],
            'f1': metrics[:,2]
